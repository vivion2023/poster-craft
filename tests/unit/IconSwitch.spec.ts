...
describe("IconSwitch component", () => {
  ...
  it("should apply the correct class based on active prop", async () => {
    // 默认为非激活状态
    expect(
      wrapper.find(".icon-container").element.classList.contains("inactive")
    ).toBeTruthy();
    expect(
      wrapper.find(".icon-container").element.classList.contains("active")
    ).toBeFalsy();
  });
  ...
  it("should emit toggle event when clicked", async () => {
    ...
    expect(toggleEvents[0]).toEqual([true]); // 从false切换到true

    // 设置为激活状态后再点击
    await wrapper.setProps({ active: true });
    await wrapper.find(".icon-switch").trigger("click");
    expect(toggleEvents[1]).toEqual([false]); // 从true切换到false
  });
  ...
  it("should apply tooltip when tooltip prop is provided", async () => {
    ...
    // 验证tooltip存在
    expect(
      tooltipWrapper.find(".ant-tooltip").exists() ||
        tooltipWrapper.find(".icon-switch").element.getAttribute("title") ===
          "Bold Text"
    ).toBeTruthy();
  });
});
